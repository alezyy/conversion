        ...
        <div class="page-content">
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li> <span>{{__('Branches')}}</span> </li>
                </ul>
            </div>
            <!-- END PAGE BAR -->
            <!-- BEGIN PAGE TITLE-->
            <h3 class="page-title">{{__('Manage Users Branches')}} <small>{{__('Branches Users')}}</small> </h3>
            <!-- END PAGE TITLE-->
            <!-- END PAGE HEADER-->
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="portlet light portlet-fit portlet-datatable bordered">
                        <div class="portlet-title">
                            ...
                            <div class="actions">
                                <a href="{{ route('create.user.branch') }}" class="btn btn-xs btn-success">
                                    <i class="glyphicon glyphicon-plus"></i> {{__('Add New User Branch')}}
                                </a>
                            </div>
                        </div>
                                        ...
                                        <tr role="row" class="filter">
                                            <td>
                                                <input type="text" class="form-control" name="name"
                                                       id="name" autocomplete="off" placeholder="User Name">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control" name="email"
                                                       id="email" autocomplete="off" placeholder="User Email">
                                            </td>
...
@push('scripts')
    <script>
        $(function () {
            var oTable = $('#branchUserDatatableAjax').DataTable({
                processing: true,
                serverSide: true,
                stateSave: true,
                searching: false,
                 ...
                 */
                ajax: {
                    url: '{!! route('fetch.data.users.branches') !!}',
                    data: function (d) {
                        d.name = $('#name').val();
                        d.email = $('#email').val();
        ...
        function deleteBranch(id) {
            ...
            if (confirm(msg)) {
                $.post("{{ route('delete.user.branch') }}", {id: id, _method: 'DELETE', _token: '{{ csrf_token() }}'})
                    .done(function (response) {
                        if (response == 'ok')
                        {
                            var table = $('#branchUserDatatableAjax').DataTable();
                            table.row('branchUserDtRow' + id).remove().draw(false);
                        } else
                        {
