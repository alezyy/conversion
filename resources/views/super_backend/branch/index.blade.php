...
@section('content')
    ...
    <div class="page-content-wrapper">
        ...
        <div class="page-content">
            ...
            <!-- END PAGE BAR -->
            <!-- BEGIN PAGE TITLE-->
            <h3 class="page-title">{{__('Manage Branches')}} <small>{{__('Branches')}}</small> </h3>
            <!-- END PAGE TITLE-->
            <!-- END PAGE HEADER-->
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="portlet light portlet-fit portlet-datatable bordered">
                        <div class="portlet-title">
                            <div class="caption"> <i class="icon-settings font-dark"></i>
                                <span class="caption-subject font-dark sbold uppercase">{{__('Branches')}}</span>
                            </div>
                            <div class="actions"> <a href="{{ route('create.branch') }}" class="btn btn-xs btn-success">
                                    <i class="glyphicon glyphicon-plus"></i> {{__('Add New Branch')}}</a>
                            </div>
                        </div>
                        <div class="portlet-body">
                            <div class="table-container">
                                <form method="post" role="form" id="datatable-search-form">
                                    <table class="table table-striped table-bordered table-hover" id="branchDatatableAjax">
                                        <thead>
                                        <tr role="row" class="filter">
                                            <td>
                                                <input type="text" class="form-control" name="name"
                                                       id="name" autocomplete="off" placeholder="Branch Name">
                                            </td>
                                            <td><input type="text" class="form-control" name="email"
                                                       id="email" autocomplete="off" placeholder="Branch Email">
                                            </td>
                                            <td>
        ...
        function makeActive(id) {
            $.post("{{ route('make.active.branch') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response === 'ok')
                    {
                        var table = $('#branchyDatatableAjax').DataTable();
                        table.row('branchDtRow' + id).remove().draw(false);
                    } else
                    {
                        alert('Request Failed!');
